...
def getSource(add):
    chooseIt()
    return netName,chainId,rpc,nativeCurrency,explorer,scanner,w3,sites('https://api.'+str(scanner)+'/api?module=contract&action=getsourcecode&address='+checkSum(str(add))+'&apikey='+str(apiKeys(scanner)))
def getRPC(js):
    if 'RPC' in js:
        return js['RPC']
    return False
