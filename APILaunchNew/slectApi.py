...
from dataFold.settings import rpcVals
...
def removeFromJs(js,ls):
  ...
  return jsN
def getStartLs():
  keys = getKeys(callJsData)
  jsN,largest = {},[0,0]
  for i in range(0,len(keys)):
      callKeys = getKeys(callJsData[keys[i]]['pieces'])
      if len(callKeys) > largest[1]:
          largest = [i,len(callKeys)]
  callKeys = getKeys(callJsData[keys[largest[0]]]['pieces'])
  ...
  for i in range(0,len(keys)):
      ...
      for k in range(0,len(callKeys)):
          ...
          if callKeys[k] not in jsN:
              jsN[callKeys[k]] = []
          if callJsData[keys[i]]['pieces'][callKeys[k]] not in jsN[callKeys[k]] and len(callJsData[keys[i]]['pieces'][callKeys[k]]) != 0:
              jsN[callKeys[k]].append(callJsData[keys[i]]['pieces'][callKeys[k]])
  return jsN
...
def buildApi():
  ...
  pen(str(removeFromJs(addJstoJs(prev,keyDrop),[0])),'prevInputs.json')
  return concateUrl(js['pieces'],keyDrop)
...
def createMix():
    js =  getStartLs()
    keys,lsNone,lsA,jsN = getKeys(js),[],[],{}
    for i in range(0,len(keys)):
        if len(js[keys[i]]) == 0:
    ...
    values = dropDown.defaultWindow(lsA,'customCreate')
    keys = getKeys(values)
    for i in range(0,len(keys)):
        if values[keys[i]] is not None:
            if len(values[keys[i]]) != 0:
                print(values[keys[i]][0])
                jsN[keys[i]] = values[keys[i]]
                if isLs(values[keys[i]]):
